    ...
    public:
        ...
        virtual void run() const
        {
            for (unsigned long size(1) ; size < (1 << 14) ; size <<= 1)
            {
                ...
                DenseVector<DataType_> sum1(VectorScaledSum<>::value(dv1, sv2, scal));
...
template <typename DataType_>
class SparseVectorDenseVectorScaledSumTest :
    public BaseTest
{
    public:
        SparseVectorDenseVectorScaledSumTest(const std::string & type) :
            BaseTest("sparse_vector_dense_vector_scaled_sum_test<" + type + ">")
        {
        }
        ...
        virtual void run() const
        {
            for (unsigned long size(1) ; size < (1 << 14) ; size <<= 1)
            {
                ...
                SparseVector<DataType_> sum1(VectorScaledSum<>::value(sv1, dv2, scal));
            ...
            TEST_CHECK_THROWS(VectorScaledSum<>::value(sv00, dv01, scal00), VectorSizeDoesNotMatch);
        }
};
SparseVectorDenseVectorScaledSumTest<float> sparse_vector_dense_vector_scaled_sum_test_float("float");
SparseVectorDenseVectorScaledSumTest<double> sparse_vector_dense_vector_scaled__sum_test_double("double");

template <typename DataType_>
class SparseVectorDenseVectorScaledSumQuickTest :
    public QuickTest
{
    public:
        SparseVectorDenseVectorScaledSumQuickTest(const std::string & type) :
            QuickTest("sparse_vector_dense_vector_scaled_sum_quick_test<" + type + ">")
        {
        }
...
SparseVectorDenseVectorScaledSumQuickTest<float> sparse_vector_dense_vector_scaled_sum_quick_test_float("float");
SparseVectorDenseVectorScaledSumQuickTest<double> sparse_vector_dense_vector_scaled__sum_quick_test_double("double");
