...
class PoissonCGBench :
    public Benchmark
{
    public:
        PoissonCGBench(const std::string & id) :
            Benchmark(id)
        {
        ...
        virtual void run()
        {
            ...
            BENCHMARK((ConjugateGradients<Tag_, methods::NONE>::value(A, b_v, result, std::numeric_limits<float>::epsilon())));
            evaluate();
        }
};
