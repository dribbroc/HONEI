...
namespace honei
{
    namespace sse
    {
        ...
        void difference(float * result, const float * a, const float * b, unsigned long size)
        {
            ...
            unsigned long b_address((unsigned long)b);
            unsigned long b_offset(b_address % 16);
            unsigned long r_address((unsigned long)result);
            unsigned long r_offset(r_address % 16);

            unsigned long x_offset(r_offset / 4);
            x_offset = (4 - x_offset) % 4;
            ...
            if (a_offset == b_offset && a_offset == r_offset)
            {
                for (unsigned long index(quad_start) ; index < quad_end ; index += 16)
                {
                    m1 = _mm_load_ps(a + index);
                    ...
                    _mm_store_ps(result + index + 12, m7);
                }
            }
            else
            {
                for (unsigned long index(quad_start) ; index < quad_end ; index += 16)
                {
                    m1 = _mm_loadu_ps(a + index);
                    m3 = _mm_loadu_ps(a + index + 4);
                    m5 = _mm_loadu_ps(a + index + 8);
                    m7 = _mm_loadu_ps(a + index + 12);
                    m2 = _mm_loadu_ps(b + index);
                    m4 = _mm_loadu_ps(b + index + 4);
                    m6 = _mm_loadu_ps(b + index + 8);
                    m8 = _mm_loadu_ps(b + index + 12);
        ...
        void difference(double * result, const double * a, const double * b, unsigned long size)
        {
            ...
            unsigned long x_offset(r_offset / 8);
            ...
            if (a_offset == b_offset && a_offset == r_offset)
            {
                for (unsigned long index(quad_start) ; index < quad_end ; index += 8)
                {
                    ...
                    _mm_store_pd(result + index + 6, m7);
                }
            }
            else
            {
                for (unsigned long index(quad_start) ; index < quad_end ; index += 8)
                {
                    m1 = _mm_loadu_pd(a + index);
                    m3 = _mm_loadu_pd(a + index + 2);
                    m5 = _mm_loadu_pd(a + index + 4);
                    m7 = _mm_loadu_pd(a + index + 6);
                    m2 = _mm_loadu_pd(b + index);
                    m4 = _mm_loadu_pd(b + index + 2);
                    m6 = _mm_loadu_pd(b + index + 4);
                    m8 = _mm_loadu_pd(b + index + 6);
