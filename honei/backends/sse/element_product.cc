...
namespace honei
{
    namespace sse
    {
        ...
        void element_product(float * a, const float * b, const float * c, unsigned long size)
        {
            ...
            unsigned long b_address((unsigned long)b);
            unsigned long b_offset(b_address % 16);
            unsigned long c_address((unsigned long)c);
            unsigned long c_offset(c_address % 16);

            unsigned long w_offset(a_offset / 4);
            ...
            if ((a_offset == b_offset) && (a_offset == c_offset))
            {
                for (unsigned long index(quad_start) ; index < quad_end ; index += 16)
                {
                    m1 = _mm_load_ps(b + index);
                    m3 = _mm_load_ps(b + index + 4);
                    m5 = _mm_load_ps(b + index + 8);
                    m7 = _mm_load_ps(b + index + 12);
                    m2 = _mm_load_ps(c + index);
                    m4 = _mm_load_ps(c + index + 4);
                    m6 = _mm_load_ps(c + index + 8);
                    m8 = _mm_load_ps(c + index + 12);
            ...
            else
            {
                for (unsigned long index(quad_start) ; index < quad_end ; index += 16)
                {
                    m1 = _mm_loadu_ps(b + index);
                    m3 = _mm_loadu_ps(b + index + 4);
                    m5 = _mm_loadu_ps(b + index + 8);
                    m7 = _mm_loadu_ps(b + index + 12);
                    m2 = _mm_loadu_ps(c + index);
                    m4 = _mm_loadu_ps(c + index + 4);
                    m6 = _mm_loadu_ps(c + index + 8);
                    m8 = _mm_loadu_ps(c + index + 12);
        ...
        void element_product(double * a, const double * b, const double * c, unsigned long size)
        {
            ...
            unsigned long w_offset(a_offset / 8);
            ...
            if ((a_offset == b_offset) && (a_offset == c_offset))
            {
                for (unsigned long index(quad_start) ; index < quad_end ; index += 8)
                {
                    m1 = _mm_load_pd(b + index);
                    m3 = _mm_load_pd(b + index + 2);
                    m5 = _mm_load_pd(b + index + 4);
                    m7 = _mm_load_pd(b + index + 6);
                    m2 = _mm_load_pd(c + index);
                    m4 = _mm_load_pd(c + index + 2);
                    m6 = _mm_load_pd(c + index + 4);
                    m8 = _mm_load_pd(c + index + 6);
            ...
            else
            {
                for (unsigned long index(quad_start) ; index < quad_end ; index += 8)
                {
                    m1 = _mm_loadu_pd(b + index);
                    m3 = _mm_loadu_pd(b + index + 2);
                    m5 = _mm_loadu_pd(b + index + 4);
                    m7 = _mm_loadu_pd(b + index + 6);
                    m2 = _mm_loadu_pd(c + index);
                    m4 = _mm_loadu_pd(c + index + 2);
                    m6 = _mm_loadu_pd(c + index + 4);
                    m8 = _mm_loadu_pd(c + index + 6);
