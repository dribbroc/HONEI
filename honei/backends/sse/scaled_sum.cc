    ...
    namespace sse
    {
        ...
        void scaled_sum(float * x, const float * y, const float * z, float b, unsigned long size)
        {
            ...
            if ((x_offset == y_offset) && (x_offset == z_offset))
            {
                for (unsigned long index(quad_start) ; index < quad_end ; index += 12)
                {
                    m1 = _mm_load_ps(y + index);
                    m3 = _mm_load_ps(y + index + 4);
                    m5 = _mm_load_ps(y + index + 8);
                    m2 = _mm_load_ps(z + index);
                    m4 = _mm_load_ps(z + index + 4);
                    m6 = _mm_load_ps(z + index + 8);
            ...
            else
            {
                for (unsigned long index(quad_start) ; index < quad_end ; index += 12)
                {
                    m1 = _mm_loadu_ps(y + index);
                    m3 = _mm_loadu_ps(y + index + 4);
                    m5 = _mm_loadu_ps(y + index + 8);
                    m2 = _mm_loadu_ps(z + index);
                    m4 = _mm_loadu_ps(z + index + 4);
                    m6 = _mm_loadu_ps(z + index + 8);
        ...
        void scaled_sum(double * x, const double * y, const double * z, double b, unsigned long size)
        {
            ...
            if ((x_offset == y_offset) && (x_offset == z_offset))
            {
                for (unsigned long index(quad_start) ; index < quad_end ; index += 6)
                {
                    m1 = _mm_load_pd(y + index);
                    m3 = _mm_load_pd(y + index + 2);
                    m5 = _mm_load_pd(y + index + 4);
                    m2 = _mm_load_pd(z + index);
                    m4 = _mm_load_pd(z + index + 2);
                    m6 = _mm_load_pd(z + index + 4);
            ...
            else
            {
                for (unsigned long index(quad_start) ; index < quad_end ; index += 6)
                {
                    m1 = _mm_loadu_pd(y + index);
                    m3 = _mm_loadu_pd(y + index + 2);
                    m5 = _mm_loadu_pd(y + index + 4);
                    m2 = _mm_loadu_pd(z + index);
                    m4 = _mm_loadu_pd(z + index + 2);
                    m6 = _mm_loadu_pd(z + index + 4);
