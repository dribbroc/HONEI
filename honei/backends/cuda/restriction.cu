...
namespace honei
{
    namespace cuda
    {
        ...
        __global__ void restriction_gpu(double * coarse, double * fine, unsigned long Nc, unsigned long Nf, unsigned long Mc, unsigned long Mf,
                unsigned long n1, unsigned long n2, unsigned long n3, unsigned long n4, unsigned long e1, unsigned long e2, unsigned long e3, unsigned long e4)
        {
            // index in the coarse array (output index)
            unsigned long ic = blockDim.x*blockIdx.x+threadIdx.x;
            ...
            // first index: 0=bottom, 1=center, 2=top
            // second index: 0=left, 1=center, 2=right
            double coeffs[3][3] = {0.25f, 0.5f, 0.25f,
                0.5f, 1.0f, 0.5f,
                0.25f, 0.5f, 0.25f};
...
#ifdef HONEI_CUDA_DOUBLE
extern "C" void cuda_restriction_double(void * coarse, unsigned long size_coarse, void * fine, unsigned long size_fine,
        unsigned long * macroBorderMask, unsigned long blocksize)
{
