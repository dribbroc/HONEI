...
namespace honei
{
    namespace opencl
    {
        void element_product_float(void * r, void * x, void * y, unsigned long size, cl_device_type type)
        {
            ...
            filename += "element_product.cl";
            kernel = OpenCLBackend::instance()->create_kernel(filename, "element_product_three_float", context, device);
        ...
        void element_product_double(void * r, void * x, void * y, unsigned long size, cl_device_type type)
        {
            ...
            filename += "/honei/backends/opencl/";
            filename += "element_product.cl";
            kernel = OpenCLBackend::instance()->create_kernel(filename, "element_product_three_double", context, device);
            clSetKernelArg(kernel, 0, sizeof(cl_mem), &r);
            clSetKernelArg(kernel, 1, sizeof(cl_mem), &x);
            clSetKernelArg(kernel, 2, sizeof(cl_mem), &y);
            clSetKernelArg(kernel, 3, sizeof(cl_uint), (void *)&size);
