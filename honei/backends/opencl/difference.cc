...
namespace honei
{
    namespace opencl
    {
        void difference_float(void *r, void * x, void * y, unsigned long size, cl_device_type type)
        {
            ...
            filename += "difference.cl";
            kernel = OpenCLBackend::instance()->create_kernel(filename, "difference_three_float", context, device);
        ...
        void difference_double(void *r, void * x, void * y, unsigned long size, cl_device_type type)
        {
            ...
            filename += "/honei/backends/opencl/";
            filename += "difference.cl";
            kernel = OpenCLBackend::instance()->create_kernel(filename, "difference_three_double", context, device);
            clSetKernelArg(kernel, 0, sizeof(cl_mem), &r);
            clSetKernelArg(kernel, 1, sizeof(cl_mem), &x);
            clSetKernelArg(kernel, 2, sizeof(cl_mem), &y);
            clSetKernelArg(kernel, 3, sizeof(cl_uint), (void *)&size);
