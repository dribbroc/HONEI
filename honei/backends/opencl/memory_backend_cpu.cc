...
{
    /**
     * \brief MemoryBackend<tags::OpenCL::CPU>::Implementation is the private implementation class for
     * MemoryBackend<tags::OpenCL::CPU>.
     *
     * \ingroup grpvector
     */
    template <> struct Implementation<MemoryBackend<tags::OpenCL::CPU> >
    {
        struct Chunk
        {
            public:
        ...
        void * upload(void * memid, void * address, unsigned long bytes)
        {
            ...
            if (i == _address_map.end())
            {
                ...
                DCQ dcq = OpenCLBackend::instance()->prepare_device(CL_DEVICE_TYPE_CPU);
        ...
        void download(void * memid, void * address, unsigned long bytes)
        {
            ...
            {
                //address not known
                throw InternalError("MemoryBackend<tags::OpenCL::CPU>::download address not found!");
            }
            else
            {
        ...
        void * alloc(void * memid, void * address, unsigned long bytes)
        {
            ...
            if (i == _address_map.end())
            {
                ...
                if (device == 0)
                    throw InternalError("MemoryBackend<tags::OpenCL::CPU>::alloc CudaMallocError!");
            ...
            {
                return i->second;
                //throw InternalError("MemoryBackend<tags::OpenCL::CPU>::alloc address already known!");
            }
        }
                ...
                void * dest_address, unsigned long bytes)
        {
            ...
            {
                throw InternalError("MemoryBackend<tags::OpenCL::CPU>::copy address not found!");
            }
            else
            {
                ...
                if (j_source->second != j_dest->second)
                    throw InternalError("MemoryBackend<tags::OpenCL::CPU>::copy src and dest on different devices!");
                ...
                {
                    if (! idle)
                        throw InternalError("MemoryBackend<tags::OpenCL::CPU>::copy Data is located on another device!");
        ...
        void fill(void * memid, void * address, unsigned long bytes, float proto)
        {
            ...
            if (i == _address_map.end())
            {
                throw InternalError("MemoryBackend<tags::OpenCL::CPU>::fill address not found!");
            }
            else
            {
                if (proto != 0)
                ...
                {
                    if (! idle)
                        throw InternalError("MemoryBackend<tags::OpenCL::CPU>::fill Data is located on another device!");
                    //running in master thread -> switch to slave thread
                    else
                    {
