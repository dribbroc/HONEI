...
namespace honei
{
    namespace opencl
    {
        void fill_float(void * x, float b, unsigned long size, cl_device_type type)
        {
            ...
            filename += "fill.cl";
            kernel = OpenCLBackend::instance()->create_kernel(filename, "fill_float", context, device);
        ...
        void fill_double(void * x, double b, unsigned long size, cl_device_type type)
        {
            cl_command_queue command_queue;
            cl_kernel kernel;
            cl_context context;
            ...
            filename += "/honei/backends/opencl/";
            filename += "fill.cl";
            kernel = OpenCLBackend::instance()->create_kernel(filename, "fill_double", context, device);
            clSetKernelArg(kernel, 0, sizeof(cl_mem), &x);
