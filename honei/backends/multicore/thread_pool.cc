...
namespace honei
{
        ...
        public Implementation<mc::ThreadPool>
    {
        ...
        std::list<std::pair<Thread *, mc::SimpleThreadFunction<mc::CASDeque<mc::ThreadTask *> > *> > _threads;
            ...
            policy(&mc::DispatchPolicy::any_core)
        {
            ...
            {
                mc::SimpleThreadFunction<mc::CASDeque<mc::ThreadTask *> > * tobj =
                    new mc::SimpleThreadFunction<mc::CASDeque<mc::ThreadTask *> >(_pool_sync, &_tasks, inst_ctr);
        ...
        ~StandardImplementation()
        {
            for(std::list<std::pair<Thread *, mc::SimpleThreadFunction<mc::CASDeque<mc::ThreadTask *> > *> >::iterator i(_threads.begin()),
        ...
        public Implementation<mc::ThreadPool>
    {
            ...
            policy(&mc::DispatchPolicy::any_core)
        {
            ...
            for (int i(_num_threads - 1) ; i >= 0 ; --i)
            {
                mc::SimpleThreadFunction<std::deque<mc::ThreadTask *> > * tobj =
                    new mc::SimpleThreadFunction<std::deque<mc::ThreadTask *> >(_pool_sync, &_tasks, inst_ctr);
                Thread * t = new Thread(*tobj);
                while (tobj->tid() == 0) ; // Wait until the thread is really setup / got cpu time for the first time
                _threads.push_back(std::make_pair(t, tobj));
    ...
    {
        /// List of user POSIX threads
        std::vector<std::pair<Thread *, mc::WorkStealingThreadFunction<std::deque<mc::ThreadTask *> > *> > _threads;
        ...
        ~WorkStealingImplementation()
        {
            ...
            for(std::vector<std::pair<Thread *, mc::WorkStealingThreadFunction<std::deque<mc::ThreadTask *> > *> >::iterator i(_threads.begin()),
                i_end(_threads.end()) ; i != i_end ; ++i)
            {
                (*i).second->stop();
                delete (*i).second;
        ...
        virtual Ticket<tags::CPU::MultiCore> * enqueue(const function<void ()> & task, mc::DispatchPolicy p)
        {
            ...
            mc::WorkStealingThreadFunction<std::deque<mc::ThreadTask *> > * wfunc(_threads[idx].second);
    ...
    {
        /// List of user POSIX threads
        std::vector<std::pair<Thread *, mc::WorkStealingThreadFunction<mc::CASDeque<mc::ThreadTask *> > *> > _threads;
            ...
            _steal_mutex(new Mutex)
        {
            ...
            {
                unsigned sched_id(affinity ? (i % (_topology->num_lpus())) : 0xFFFF);
                mc::WorkStealingThreadFunction<mc::CASDeque<mc::ThreadTask *> > * tobj = new mc::WorkStealingThreadFunction<mc::CASDeque<mc::ThreadTask *> >(_pool_sync, inst_ctr, sched_id, _threads, _num_threads, global_terminate, _steal_mutex);
                Thread * t = new Thread(*tobj);
                while (tobj->tid() == 0) ; // Wait until the thread is really setup / got cpu time for the first time
                _threads.push_back(std::make_pair(t, tobj));
        ...
        ~WorkStealingImplementation()
        {
            ...
            for(std::vector<std::pair<Thread *, mc::WorkStealingThreadFunction<mc::CASDeque<mc::ThreadTask *> > *> >::iterator i(_threads.begin()),
                i_end(_threads.end()) ; i != i_end ; ++i)
            {
                (*i).second->stop();
                delete (*i).second;
        ...
        virtual Ticket<tags::CPU::MultiCore> * enqueue(const function<void ()> & task, mc::DispatchPolicy p)
        {
            ...
            mc::WorkStealingThreadFunction<mc::CASDeque<mc::ThreadTask *> > * wfunc(_threads[idx].second);
            wfunc->enqueue(t_task);
