        ...
        for( ; ar < a_rows ; ar++)
        {
            ...
            while (b_counter <= nr_of_lists - 1) // db for Lists of B
            {
                ...
                for (unsigned br(0) ; br < b_nr_rows ; br++, a_elem++)
                {
                    ...
                    if (b_vecs % 2 != 0)
                    {
                        vector double temp = b[b_current].vectorised[b_vec_idx - 1]; // temp version needed, cause original matrix must not be changed!
                        extract(temp, b[b_current].vectorised[b_vec_idx], e_offset);
                        r_temps[b_vecs] = spu_madd(a_vec, temp, r_temps[b_vecs]);
                        b_vec_idx--;
                    }
