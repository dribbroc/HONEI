...
namespace honei
{
    ...
    {
        template <>
        unsigned operation<Operation<1, double, rtm_mail> >(const Operation<1, double, rtm_mail> & operation,
                const Instruction & instruction)
        {
            ...
            while (counter > 1)
            {
                ...
                acc.value = operation.calculate(acc.value, a[current].vectorised, size / sizeof(vector double), scalar);
