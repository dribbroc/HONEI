...
class PoissonTestJACDenseDouble:
    public BaseTest
{
    public:
        PoissonTestJACDenseDouble(const std::string & tag) :
            BaseTest("Poisson test for itrerative LES solvers , JAC (dense system)<" + tag + ">")
        {
        ...
        virtual void run() const
        {
            ...
            Jacobi<Tag_>::value(A, b_v, result, std::numeric_limits<double>::epsilon());
            //std::cout<< result <<endl;
            //std::cout<< ana_sol_v <<endl;
            //std::cout<< ref_sol_v <<endl;
            for(int i = 0; i < n; i++)
            {
                TEST_CHECK_EQUAL_WITHIN_EPS(ref_sol_v[i], result[i], 1e-5);
            }
