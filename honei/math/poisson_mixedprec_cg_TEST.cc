...
class PoissonTestCGMIXEDPREC:
    public BaseTest
{
    public:
        PoissonTestCGMIXEDPREC(const std::string & tag) :
        ...
        virtual void run() const
        {
            ...
            result = ConjugateGradients<Tag_, NONE>::value(A, b_v, std::numeric_limits<float>::epsilon(), 20);
...
PoissonTestCGMIXEDPREC<tags::CPU, float> poisson_test_cg_MIXEDPREC("float");
#ifdef HONEI_SSE
PoissonTestCGMIXEDPREC<tags::CPU::SSE, float> poisson_test_cg_MIXEDPREC_sse("SSE float");
#endif
#ifdef HONEI_CELL
PoissonTestCGMIXEDPREC<tags::Cell, float> poisson_test_cg_MIXEDPREC_cell("Cell float");
#endif
