...
class PoissonTestJACKernelCascadeBandedFloat:
    public BaseTest
{
    public:
        PoissonTestJACKernelCascadeBandedFloat(const std::string & tag) :
        ...
        virtual void run() const
        {
            ...
            while(fabs(norm_x - norm_x_last) > konv_rad)
            {
                ...
                JacobiKernelCascade<Tag_, ONCE, NONE>::value(b_v, x, diag_inverted, difference, scaled_diag_inverted);
...
PoissonTestJACKernelCascadeBandedFloat<tags::CPU, float> poisson_test_jac_banded_float("float");

template <typename Tag_, typename DT1_>
class PoissonTestJACKernelCascadeBandedFloatQ1:
    public BaseTest
{
    public:
        PoissonTestJACKernelCascadeBandedFloatQ1(const std::string & tag) :
            BaseTest("Poisson test for itrerative LES solvers , JAC Kernel(banded system) <" + tag + ">")
        {
            register_tag(Tag_::name);
        }
        ...
        virtual void run() const
        {
            ...
            while(fabs(norm_x - norm_x_last) > konv_rad)
            {
                ...
                x = JacobiKernelCascade<Tag_, ONCE, Q1>::value(b_v, x, diag_inverted, difference, scaled_diag_inverted);
                norm_x = Norm<vnt_l_two, false, Tag_>::value(x);
                norm_x_last = Norm<vnt_l_two, false, Tag_>::value(x_last);
                x_last = x.copy();
            }
...
PoissonTestJACKernelCascadeBandedFloatQ1<tags::CPU, float> poisson_test_jac_banded_float_q1(" Q1 float");

template <typename Tag_, typename DT1_>
class PoissonTestJACKernelCascadeBandedFloatQ1TWICE:
    public BaseTest
{
    public:
        PoissonTestJACKernelCascadeBandedFloatQ1TWICE(const std::string & tag) :
            BaseTest("Poisson test for itrerative LES solvers , JAC Kernel(banded system) <" + tag + ">")
        {
            register_tag(Tag_::name);
        }
        ...
        virtual void run() const
        {
            ...
            while(fabs(norm_x - norm_x_last) > konv_rad)
            {
                ...
                x = JacobiKernelCascade<Tag_, TWICE, Q1>::value(b_v, x, diag_inverted, difference, scaled_diag_inverted);
                norm_x = Norm<vnt_l_two, false, Tag_>::value(x);
                norm_x_last = Norm<vnt_l_two, false, Tag_>::value(x_last);
                x_last = x.copy();
            }
...
PoissonTestJACKernelCascadeBandedFloatQ1TWICE<tags::CPU, float> poisson_test_jac_banded_float_q1_2(" Q1 float");
