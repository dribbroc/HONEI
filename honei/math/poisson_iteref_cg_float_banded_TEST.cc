...
class PoissonTestIterefCGBandedFloat:
    public BaseTest
{
    public:
        PoissonTestIterefCGBandedFloat(const std::string & tag) :
            BaseTest("Poisson test for itrerative LES solvers , Iterative Refinement CG (banded system)<" + tag + ">")
        {
        ...
        virtual void run() const
        {
            ...
            IterativeRefinement<methods::CG, Tag_>::value(A, b_v, result, std::numeric_limits<double>::epsilon(), std::numeric_limits<double>::epsilon());
...
#ifdef HONEI_SSE
PoissonTestIterefCGBandedFloat<tags::CPU::SSE, float> poisson_test_iteref_cg_banded_float_sse("SSE float");
#endif
#ifdef HONEI_CELL
PoissonTestIterefCGBandedFloat<tags::Cell, float> poisson_test_cg_banded_float_cell("Cell float");
#endif
