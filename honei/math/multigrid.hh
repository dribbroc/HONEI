...
namespace honei
{
    ...
    class Multigrid<Tag_, OuterTag_, ProlType_, methods::JAC, methods::CYCLE::V, methods::FIXED>
    {
        ...
        static DenseVector<Prec_> _multigrid_kernel(HONEI_UNUSED BandedMatrixQx<Q1Type, Prec_>&  system, HONEI_UNUSED DenseVector<Prec_>& right_hand_side, unsigned long max_levels, Prec_ * cappa, MGInfo<Prec_, BandedMatrixQx<Q1Type, Prec_> > & info)
        {
...
endCycleLoop:
        ...
        public:
        template<typename Prec_>
        static inline void value(HONEI_UNUSED BandedMatrixQx<Q1Type, Prec_>&  system,
...
endCycleLoop:
    ...
    template<typename Tag_, typename OuterTag_, typename ProlType_>
    class Multigrid<Tag_, OuterTag_, ProlType_, methods::ILU, methods::CYCLE::V, methods::FIXED>
    {
        template <typename Prec_>
        static DenseVector<Prec_> _multigrid_kernel(HONEI_UNUSED SparseMatrixELL<Prec_>&  system, HONEI_UNUSED DenseVector<Prec_>& right_hand_side, unsigned long max_levels, Prec_ * cappa, MGInfo<Prec_, SparseMatrixELL<Prec_> > & info)
        {
                    ...
                    while(true)
                    {
                        ...
                        while(true)
                        {
                            ...
                            if (restriction_started)
                            {
                                ...
                                //(info.c[current_level]) = (Jacobi<Tag_>::value(info.a[current_level], info.d[current_level], Prec_(0.7), info.diags_inverted[current_level]));
                                //Jacobi<Tag_>::value(info.a[current_level], info.d[current_level], (info.c[current_level]), Prec_(0.7), info.diags_inverted[current_level]);
                                SuperILU::value(info.a[current_level], info.d[current_level], info.c[current_level]);
                            ...
                            else
                            {
                                ...
                                //info.x[current_level] = (Jacobi<Tag_>::value((info.a[current_level]), (info.d[current_level]), Prec_(0.7), info.diags_inverted[current_level]));
                                //Jacobi<Tag_>::value((info.a[current_level]), (info.d[current_level]), info.x[current_level], Prec_(0.7), info.diags_inverted[current_level]);
                                SuperILU::value(info.a[current_level], info.d[current_level], info.x[current_level]);
                                //END NEWTEST
                            }
                            DenseVector<Prec_> defect_2(Defect<Tag_>::value(info.rhs[current_level], info.a[current_level], info.x[current_level]));
                            info.d[current_level] = defect_2;
...
endRestrictionLoop:
                        ...
                        if (info.min_level == info.max_level)
                        {
                            ...
                            //(info.x[current_level]) =(ConjugateGradients<Tag_, NONE>::value((info.a[current_level]), (info.d[current_level]), std::numeric_limits<Prec_>::epsilon()));
                            //ConjugateGradients<Tag_, methods::NONE>::value((info.a[current_level]), (info.d[current_level]), info.x[current_level], std::numeric_limits<Prec_>::epsilon());
                            SuperLU::value(info.a[current_level], info.d[current_level], info.x[current_level]);
                        ...
                        while (true)
                        {
                            ...
                            //(info.x[current_level]) =(Jacobi<Tag_>::value(info.x[current_level], (info.a[current_level]), (info.rhs[current_level]), info.n_pre_smooth, Prec_(0.7), info.diags_inverted[current_level]));
                            Jacobi<Tag_>::value(info.x[current_level], (info.a[current_level]), (info.rhs[current_level]),(info.x[current_level]), info.n_post_smooth, Prec_(0.7), info.diags_inverted[current_level]);
                            //SuperILU::value(info.a[current_level], info.rhs[current_level], info.x[current_level]);
                            //end NEWTEST
#ifdef SOLVER_VERBOSE
                            std::cout << "Postsmoothing ||X|| on level " << current_level << " " << Norm<vnt_l_two, true, Tag_>::value(info.x[current_level]) << std::endl;
#endif
...
endCycleLoop:
                ...
                MGInfo<Prec_, SparseMatrixELL<Prec_> > & info)
        {
            CONTEXT("When solving sparse ELL linear system with MULTIGRID: ");
#ifdef SOLVER_VERBOSE_L2
            std::cout << "Calling MG solver, presmoothing=ILU, postsmoothing=ILU, coarse-grid solver=LU, datalayout=ELLPACK, precision=FIXED" << std::endl;
#endif
            Prec_ cappa;
    ...
    {
        template <typename Prec_>
        static DenseVector<Prec_> _multigrid_kernel(unsigned long max_levels, Prec_ * cappa, MGInfo<Prec_, BandedMatrixQx<Q1Type, Prec_> > & info)
        {
            bool restriction_started(false);
            // compute initial defect
            // when start vector is zero: set d = rhs (we can save one matvec here)
...
endCycleLoop:
        ...
        static inline void value(BandedMatrixQx<Q1Type, OuterPrec_>&  system,
                DenseVector<OuterPrec_>& right_hand_side,
                DenseVector<OuterPrec_>& x,
                unsigned long max_levels,
                HONEI_UNUSED OuterPrec_ conv_rad,
                MGInfo<InnerPrec_, BandedMatrixQx<Q1Type, InnerPrec_> > & info)
            {
                CONTEXT("When solving banded q1 linear system with MULTIGRID: ");
#ifdef SOLVER_VERBOSE_L2
...
//------------------MIXED-----SPAI--------------------------------
    ...
    template<typename Tag_ , typename OuterTag_, typename ProlType_>
    class Multigrid<Tag_ , OuterTag_ , ProlType_, methods::SPAI, methods::CYCLE::V, methods::MIXED>
    {
        private:
        template <typename Prec_>
        static DenseVector<Prec_> _multigrid_kernel(unsigned long max_levels, Prec_ * cappa, MGInfo<Prec_, SparseMatrixELL<Prec_> > & info)
...
endCycleLoop:
        ...
        public:
                ...
                MGInfo<InnerPrec_, SparseMatrixELL<InnerPrec_> > & info)
            {
                CONTEXT("When solving banded q1 linear system with MULTIGRID: ");
#ifdef SOLVER_VERBOSE_L2
                std::cout << "Calling MG solver, presmoothing=SPAI, postsmoothing=SPAI, coarse-grid solver=CG, datalayout=ELL, precision=MIXED" << std::endl;
#endif
#ifdef SOLVER_BENCHMARK
                TimeStamp ab, ae;
                ab.take();
