...
namespace
{
    ...
    class cudaColStreamGriddouble
    {
        private:
            ...
            unsigned long blocksize;
        public:
            cudaColStreamGriddouble(PackedGridInfo<D2Q9> & info, PackedGridData<D2Q9, double> & data, double tau, unsigned long blocksize) :
                info(info),
                data(data),
                tau(tau),
                blocksize(blocksize)
            ...
            void operator() ()
            {
                ...
                cuda_collide_stream_grid_double(start, end,
                        cuda_dir_1_gpu, cuda_dir_2_gpu, cuda_dir_3_gpu, cuda_dir_4_gpu,
                        cuda_dir_5_gpu, cuda_dir_6_gpu, cuda_dir_7_gpu, cuda_dir_8_gpu,
                        f_eq_0_gpu, f_eq_1_gpu, f_eq_2_gpu,
                        f_eq_3_gpu, f_eq_4_gpu, f_eq_5_gpu,
             ...
             double tau)
{
    ...
    if (! cuda::GPUPool::instance()->idle())
    {
        cudaColStreamGriddouble task(info, data, tau, blocksize);
        task();
    }
    else
    {
