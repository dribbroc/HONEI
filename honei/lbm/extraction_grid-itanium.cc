...
void ExtractionGrid<tags::CPU::Itanium, lbm_modes::WET>::value(
        PackedGridInfo<D2Q9> & info, PackedGridData<D2Q9, DT1_> & data, DT1_ epsilon)
{
    CONTEXT("When extracting h, u and v(Itanium):");

    //set f to t_temp
    DenseVector<DT1_> * swap;
    swap = data.f_0;
    ...
    itanium::extraction_grid_wet(begin, end,
...
template void ExtractionGrid<tags::CPU::Itanium, lbm_modes::WET>::value<float>(
        PackedGridInfo<D2Q9> &, PackedGridData<D2Q9, float> &, float);
template void ExtractionGrid<tags::CPU::Itanium, lbm_modes::WET>::value<double>(
