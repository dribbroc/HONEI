...
template <typename Tag_, typename DT1_>
class CGSolverTestSparseCSR:
    public BaseTest
{
    private:
        std::string _m_f, _v_f, _r_f, _i_f, _p_f;
    public:
        CGSolverTestSparseCSR(const std::string & tag,
                std::string m_file,
                std::string v_file,
                std::string res_file,
                std::string init_file,
                std::string precon_file) :
            BaseTest("MPI CG solver test (sparse CSR system)<" + tag + ">")
        {
            register_tag(Tag_::name);
            _m_f = m_file;
            _v_f = v_file;
        ...
        virtual void run() const
        {
            ...
            filename_5 += "/honei/math/testdata/poisson_advanced2/q2_sort_0/";
            filename_5 += _p_f;
            SparseMatrixCSR<DT1_> sdiag_inverted_ell(MatrixIO<io_formats::ELL>::read_matrix(filename_5, DT1_(0)));
            SparseMatrixCSR<DT1_> sdiag_inverted(sdiag_inverted_ell);
            SparseMatrixCSRMPI<DT1_> diag_inverted(sdiag_inverted);
