...
class PoissonTestJACBandedFloat:
    public BaseTest
{
    public:
        PoissonTestJACBandedFloat(const std::string & tag) :
            BaseTest("Poisson test for itrerative LES solvers , JAC (banded system)<" + tag + ">")
    {
...
PoissonTestJACBandedFloat<tags::CPU, float> poisson_test_jac_banded_float("float");
#ifdef HONEI_SSE
