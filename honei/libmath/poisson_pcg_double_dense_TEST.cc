...
class PoissonTestPCGDenseDouble:
    public BaseTest
{
    public:
        PoissonTestPCGDenseDouble(const std::string & tag) :
            BaseTest("Poisson test for itrerative LES solvers , PCG (dense system)<" + tag + ">")
        {
        ...
        virtual void run() const
        {
            ...
            result = ConjugateGradients<Tag_, JAC>::value(A, b_v, std::numeric_limits<double>::epsilon());
...
PoissonTestPCGDenseDouble<tags::CPU, double> poisson_test_pcg_dense_double("double");
#ifdef HONEI_SSE
