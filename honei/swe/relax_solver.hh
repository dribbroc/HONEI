...
using namespace precision_modes;
    ...
    template<typename Tag_,
             ...
             typename InitPrec1_,
             typename InitPrec2_>
    class RelaxSolver<Tag_, ResPrec_, PredictionPrec1_, PredictionPrec2_, InitPrec1_, InitPrec2_, source_types::SIMPLE, REFLECT, MIXED>
    {
        ///Private members.
        private:

            //-------------------------------------------
            DenseVector<PredictionPrec2_>* _bx_2;
            DenseVector<PredictionPrec2_>* _by_2;
            //-------------------------------------------
             ...
             **/
            template<typename WorkPrec_>
                void _do_prediction_2(DenseVector<WorkPrec_>& predictedu, DenseVector<WorkPrec_>& predictedv, DenseVector<WorkPrec_>& predictedw)
                {
                    BandedMatrix<WorkPrec_> m1(_u->size());
                    BandedMatrix<WorkPrec_> m2(_u->size());
                    BandedMatrix<WorkPrec_> m3(_u->size());
...
#endif
                    //------------------------------------------
                    WorkPrec_ dx(_delta_x);
                    WorkPrec_ dy(_delta_y);
                    WorkPrec_ dt(_delta_t);

                    DenseVector<WorkPrec_> c_2(_c->size());
                    DenseVector<WorkPrec_> d_2(_d->size());
                    convert(c_2, *_c);
                    convert(d_2, *_d);
                    //------------------------------------------
                    AssemblyProcessing<Tag_, MAIN::M1M3>::value(m1, m3, predictedu, predictedv, dt, dx, _d_width, _d_height, c_2);
                    AssemblyProcessing<Tag_, MAIN::M2M4>::value(m2, m4, predictedu, predictedw, dt, dy, _d_width, _d_height, d_2);

                    BandedMatrix<WorkPrec_> m5c(m3.copy());

                    BandedMatrix<WorkPrec_> m6(_u->size());
                    AssemblyProcessing<Tag_, QUICK::M6>::value(m1, m6, c_2, _d_width, _d_height);
                    ...
                    //--------------------------------------------
                    WorkPrec_ mn_2(_manning_n_squared);
                    //--------------------------------------------
                    SourceProcessing<source_types::SIMPLE, Tag_>::value(source_c, *_bx_2, *_by_2, mn_2);
        ...
        public:
             ...
             *
             **/
            RelaxSolver(Scenario<ResPrec_, swe_solvers::RELAX, REFLECT> & scenario, DenseVector<PredictionPrec2_> & bx, DenseVector<PredictionPrec2_> & by )
            {
                this->scenario = &scenario;
                this->_height = scenario.height;
                this->_bottom = scenario.bottom;
            ...
            void solve()
            {
                ...
                _do_setup_stage2<InitPrec2_>(predictedu_c, predictedv_c, predictedw_c);
                //-------------------------------------
                DenseVector<PredictionPrec2_> predictedu_c_2(predictedu_c.size());
                DenseVector<PredictionPrec2_> predictedv_c_2(predictedv_c.size());
                DenseVector<PredictionPrec2_> predictedw_c_2(predictedw_c.size());

                convert(predictedu_c_2, predictedu_c);
                convert(predictedv_c_2, predictedv_c);
                convert(predictedw_c_2, predictedw_c);

                _do_prediction_2<PredictionPrec2_>(predictedu_c_2, predictedv_c_2, predictedw_c_2);

                convert(predictedu_c, predictedu_c_2);
                convert(predictedv_c, predictedv_c_2);
                convert(predictedw_c, predictedw_c_2);
                //-------------------------------------
                CorrectionProcessing<REFLECT, Tag_>::value(predictedu_c, predictedv_c, predictedw_c, *_u, *_v, *_w, _d_width, _d_height, *_height);
                ++_solve_time;
