...
namespace honei
{
    ...
    template <> struct DotProduct<tags::CPU>
    {
        ...
        template <typename DT1_, typename DT2_>
        static inline BenchmarkInfo get_benchmark_info(const DenseVectorContinuousBase<DT1_> & a, const DenseVectorContinuousBase<DT2_> & b)
        {
            BenchmarkInfo result;
            result.flops = 2 * a.size();
            result.load = a.size() * (sizeof(DT1_) + sizeof(DT2_));
    ...
    template <> struct DotProduct<tags::CPU::Generic>
    {
        ...
        static inline DT_ value(const DenseVectorContinuousBase<DT_> & x, const DenseVectorContinuousBase<DT_> & y)
        {
            ...
            BENCHADD(DotProduct<tags::CPU>::get_benchmark_info(x, y));
    ...
    template <> struct DotProduct<tags::GPU::CUDA>
    {
        ...
        static inline DT_ value(const DenseVectorMPI<DT_> & x, const DenseVectorMPI<DT_> & y)
        {
            return MPIOps<tags::GPU::CUDA>::dot_product(x, y);
        }
        /// \}
    };
    ...
    namespace mc
    {
        template <typename Tag_> struct DotProduct
        {
            ...
            static inline DT_ value(const DenseVectorMPI<DT_> & x, const DenseVectorMPI<DT_> & y)
            {
                return MPIOps<Tag_>::dot_product(x, y);
            }
        };
    }
